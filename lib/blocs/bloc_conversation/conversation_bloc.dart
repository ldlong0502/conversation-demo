  ...
  ConversationBloc() : super(ConversationInitial()) {
    ...
    on<UpdateTimeHighLight>((event, emit) async {
      if (state is ConversationLoaded) {
        try {
          var index = (state as ConversationLoaded)
              .listConversations
              .indexWhere(
                  (e) => checkHighLight(event.timeHighLight, e.start, e.end));
          if (index != -1) {
            final isHighLight = (state as ConversationLoaded)
            ...
            if (!isHighLight &&
                !checkIndexInItemPositions(
                    index, (state as ConversationLoaded).itemPositions)) {
              event.scrollController.scrollTo(
                index: index + 1 > (state as ConversationLoaded).listConversations.length ? index : index +1,
                alignment: 0.8,
            ...
            if (event.timeHighLight == 0) {
              event.scrollController.jumpTo(
                index: 0,
                // Replace `itemHeight` with the height of each conversation item
      ...
      if (state is ConversationLoaded) {
        try {
          if (event.index == 1) {
            ...
            await consRepo
                .setTranslate(!(state as ConversationLoaded).isTranslate);
          } else if (event.index == 3) {
            await consRepo
                .setPhonetic(!(state as ConversationLoaded).isPhonetic);
          }
          emit(ConversationLoaded(
            listConversations: (state as ConversationLoaded).listConversations,
            timePosition: (state as ConversationLoaded).timePosition,
            audioPlayer: (state as ConversationLoaded).audioPlayer,
            isPlaying: (state as ConversationLoaded).isPlaying,
            isBlur: event.index == 1
                ? !(state as ConversationLoaded).isBlur
                : (state as ConversationLoaded).isBlur,
            isSpeed: event.index == 4
                ? !(state as ConversationLoaded).isSpeed
                : (state as ConversationLoaded).isSpeed,
            isTranslate: event.index == 2
                ? !(state as ConversationLoaded).isTranslate
                : (state as ConversationLoaded).isTranslate,
            isPhonetic: event.index == 3
                ? !(state as ConversationLoaded).isPhonetic
                : (state as ConversationLoaded).isPhonetic,
            itemPositions: (state as ConversationLoaded).itemPositions,
          ));
        } catch (e) {
